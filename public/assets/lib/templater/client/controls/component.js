        ...
        if (!Component.typeDialog) {
            ...
            for (var cat in by_cat) {
                ...
                $.each(by_cat[cat],function(){
                    var type = this;
                    var item = 
                        $("<div class='combo-item draggable'>").addClass('type-'+type.id).html(type.name).data("component",{create:true,type:type})
                        .mousedown(function(e){
                            var handle = Component.typeDialog.handle;
                            // if it's create (not change type) and no children there is no sense in DnD, so just trigger create below
                            if (handle && Component.previewFrame.root.children.length==0) {
                                e.preventDefault();
                                e.stopPropagation();
                                $(this).click();
                                return false;
