...
browser.initUploadButton = function() {
    ...
    uploader.bind('FilesAdded', function(up, files) {
        ...
        $('#loading').html(me.label("Uploading file..."));
        $('#loading').css('display', 'inline');
        
        uploader.start();
        count = uploader.total.queued;
    });    
    
    uploader.bind("FileUploaded",function(){
        var response = arguments[2].response;
    ...
    uploader.bind("UploadProgress",function(up,file) {
        var part = 100.0 / count;
        var progress = (count - uploader.total.queued) * part + (part * file.percent / 100);
        progress = Math.floor(progress);
        
        $('#loading').html(me.label("Uploading file...") + " "+progress+"%");
    });
