...
(function ($, undefined) {
    ...
    $.fn.flexbeSlider = function (options) {
        ...
        plugin = {
            ...
            controls: function () {
                if (settings.controls) {
                    $slide.after('<div class="slider_act"><a class="slider_prev">' + settings.prevHtml + '</a><a class="slider_next">' + settings.nextHtml + '</a></div>');
                    var $prev = $slide.parent().find('.slider_prev');
                    var $next = $slide.parent().find('.slider_next');
            ...
            pager: function () {
                ...
                refresh.createPager = function () {
                    ...
                    for (i = 0; i < maxSlide; i++) {
                        ...
                        if (settings.gallery === true) {
                            pagers += '<li style="float:left;width:' + settings.thumbWidth + 'px;margin-right:' + settings.thumbMargin + 'px"><a><img src="' + thumb + '" /></a></li>';
                        } else {
                            pagers += '<li><a>' + (i + 1) + '</a></li>';
                        }
                        if (settings.mode === 'slide') {
                    ...
                    $cSouter.find('.slider_pager').html(pagers);
                    if (settings.gallery === true) {
                        pagerWidth = i * (settings.thumbMargin + settings.thumbWidth);
                        $cSouter.find('.slider_pager').css({
                            'width': pagerWidth + 'px',
                            'transform': 'translate3d(0px, 0px, 0px)',
                            'transition': '1s all'
                        });
                    }
                    var $pager = $cSouter.find('.slider_pager').find('li');
                ...
                if (settings.pager) {
                    ...
                    $slide.after('<ul class="slider_pager ' + cl + '"></ul>');
                    refresh.createPager();
                }
            ...
            move: function (ob, v) {
                ...
                var $thumb = $slide.parent().find('.slider_pager').find('li');
            ...
            slideThumb: function () {
                ...
                var $pager = $slide.parent().find('.slider_pager');
        ...
        $el.setSettings = function (opts) {
            if( typeof(opts) === "object" )
            {
                var is_refresh = false;
                $.each(opts,function(opt_name,opt_value)
                {
                    if( settings[opt_name] !== opt_value )
                    {
                        settings[opt_name] = opt_value;
                        is_refresh = true;
                    }
                });
                if( is_refresh ) { refresh.init(); }
            }
        };
